...
async function openGeneListPopup(conceptId, element) {
    ...
    // Checking if modal element is already created for current conceptID
        ...
        const getCachedData = await cacheManager.getCachedData(request); 
        ...
        if(!getCachedData){
            ...
            {   
                ...
                createGenesColPopup(description, data, type, getTaxIdFrag,conceptId); 
            
            }).fail(function (xhr, status, errolog) {
                jboxNotice('An error occured, kindly try again', 'red', 300, 2000);
            })
        }else{
            createGenesColPopup(description, getCachedData, type, getTaxIdFrag,conceptId);
        }
