...
function createGenesTable(text, keyword, rows) {
	...
	$(".viewGeneNetwork").bind("click", { x: candidateGenes }, function (e) {
		...
		generateCyJSNetwork(api_url + '/network', { list: [values[1]], keyword: keyword, exportPlainJSON: false }, true);
	});
 ...
 * @author: Ajit Singh.
 */
function generateCyJSNetwork(url, requestParams,position) {
	...
	$.post({
		...
		try {
			...
			if (data.graph.includes("var graphJSON=")) { // for old/current json that contains 2 JS vars
				...
				knetmaps.drawRaw('#knet-maps',knetwork_blob);
...
function generateMultiGeneNetwork_forNewNetworkViewer(keyword) {
	...
	if (candidatelist == "") {
		...
		generateCyJSNetwork(api_url + '/network', { keyword: keyword, list: candidatelist, exportPlainJSON: false }, true);
	}
